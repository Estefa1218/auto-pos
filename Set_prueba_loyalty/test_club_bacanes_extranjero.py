...
fake = Faker('es_CO') 
DOCUMENTO = str(random.randint(300000000, 999999999))
...
TELEFONO = f"09{fake.random_number(digits=7, fix_len=True)}"#aqui el cambio de telefono
def test_nuevo_cliente_club_bacanes_extranjero():
    pos = setup_and_login(f"Orden con registro de nuevo cliente club bacanes extranjero: {NOMBRE}")
    if pos:
        try:
            exito = True
            ...
            if exito: exito = pos.registrar_cliente_en_orden_extranjero(DOCUMENTO, NOMBRE, APELLIDO, CORREO, TELEFONO)
            ...
            if pos.pagar_con_efectivo("14000", "648"):
                ...
                if exito:
                    assert True, "El escenario de Orden con registro de nuevo cliente club bacanes extranjero finalizó con éxito."
